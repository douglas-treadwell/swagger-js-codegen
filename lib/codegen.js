...
var getViewForSwagger2 = function(opts, type){
    ...
    _.forEach(swagger.paths, function(api, path){
        ...
        _.forEach(api, function(op, httpMethod){
            ...
            // extract response type for 200 or 201 response, if present
            if ( _.isObject(op.responses) && ( _.isObject(op.responses['200']) || _.isObject(op.responses['201']) ) ) {
                var resp = op.responses['200'] || op.responses['201'];

                if (resp.schema) {
                    method.responseType = typeConversion.convertType(resp);

                    if ( method.responseType.tsType === 'object' ) { // create types for returned objects
                        method.responseType.name = method.methodName.charAt(0).toUpperCase() + method.methodName.substr(1) + 'Response';
                        data.definitions.push(method.responseType);

                        // replace the original method return signature with the new type signature
                        method.responseType = typeConversion.convertType({ $ref: method.responseType.name });
                    }
                }
            }
