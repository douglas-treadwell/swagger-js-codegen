...
function convertType(swaggerType) {
    ...
    if (swaggerType.hasOwnProperty('schema')) {
        ...
        if ( !swaggerType.enum ) {
            ...
            var strings = _.map(swaggerType.enum, function(enumString) { return '\'' + enumString + '\''; });
            typespec.tsType = strings.join(' | ');
    ...
    } else if (swaggerType.type === 'object' || swaggerType.properties ) { // if schema has properties, assume type is object
        ...
        _.forEach(swaggerType.properties, function (propertyType, propertyName) {
            ...
            property.isOptional = propertyType.hasOwnProperty('required') && propertyType.required === false;
            property.isRequired = !property.isOptional; // inverse for convenience
    ...
    typespec.isAtomic = typespec.isAtomic || _.contains(['string', 'number', 'boolean', 'any'], typespec.tsType);
